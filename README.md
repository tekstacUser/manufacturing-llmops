...
models must already be present under `models_app/` before you start (see each
`models_app/*/README.md`).
...
├── models_app/                  Pre-provisioned GGUF files go here (not downloaded)
...
1. Confirm the model files exist under `models_app/qwen-0.5b/`,
   `models_app/qwen-1.5b/`, and `models_app/qwen-1.5b-q4/` (see each folder's README).
...
hf download Qwen/Qwen2.5-0.5B-Instruct-GGUF \
  ...
  --local-dir models_app/qwen-0.5b

hf download Qwen/Qwen2.5-1.5B-Instruct-GGUF \
  qwen2.5-1.5b-instruct-fp16.gguf \
  --local-dir models_app/qwen-1.5b

hf download Qwen/Qwen2.5-1.5B-Instruct-GGUF \
  qwen2.5-1.5b-instruct-q4_k_m.gguf \
  --local-dir models_app/qwen-1.5b-q4
...
|---|---|---|---|
| `SMALL_MODEL_PATH`, `MEDIUM_MODEL_PATH`, `QUANTIZED_MODEL_PATH` | Container-internal path to each GGUF file | These are pre-provisioned for you — confirm the filenames under `models_app/*/` match these paths exactly | Just make sure the file exists at that path before `docker compose up` |
